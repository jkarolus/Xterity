import wx
from gui.StreamOverviewPanel import StreamOverviewPanel
...
class MainFrame(wx.Frame):
    ...
    def OnExit(self, event):
        if self.streamPanel.streamManager.isRecording():
            diag = wx.MessageDialog(None, "Are you sure you want to quit?\nOngoing recordings will not be saved!", "Ongoing recording detected", wx.YES_NO | wx.NO_DEFAULT | wx.ICON_WARNING).ShowModal()
            if diag != wx.ID_YES:
                return
        self.Destroy()
